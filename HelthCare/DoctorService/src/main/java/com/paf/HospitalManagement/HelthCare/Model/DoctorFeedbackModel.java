...
//Create method for ViewDoctorFeedback
	...
	public String UpdateDoctorFeedback(DoctorFeedbackBean Dbean) {
		...
		try {
			// create a prepared statement    
				...
				outputs = "Doctor Feedback details Updated successfully";
				
				
			}catch (SQLException e) {
	
				outputs = "Error while reading the DoctorFeedback.";
				System.err.println(e.getMessage());
			}
	...
	public String DeleteDoctorFeedback(int id) {
			...
			try {
					...
					// execute the statement  
					preparedStmt.execute();			
					output = "doctorfeedback "+id+" Doctor Feedback Deleted successfully ";//display delete successfully
